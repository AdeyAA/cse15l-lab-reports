...
For this step, I used the `CTRL-R` command and then pressed `<tab>` to complete the line `ssh aaali@ieng6.ucsd.edu` in the terminal then pressed `<enter>`.
...
-I copied the ssh url from GitHub using `CTRL-C` then just typed `git clone` then used `CTRL-P` then `<enter>`.
...
-To add the changed commit and push the changes to Github I used the `CTRL-R` for all of these steps until the command I was looking for was semi auto-completed then used the `<tab>` key then pressed `<enter>`.
